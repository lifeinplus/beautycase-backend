...
describe("RefreshController GET", () => {
    ...
    it("should handle token reuse and clear hacked user tokens", async () => {
        ...
        const user = await UserModel.findOne({ username: mockHacker.username });
    ...
    it("should refresh token and return new tokens", async () => {
        ...
        expect(response.body).toHaveProperty("username", mockUser1.username);
    });
    ...
    test("should return 401 when username in token doesn't match", async () => {
        const refreshToken = jwt.sign(
            { username: mockUser1.username },
            config.auth.refreshToken.secret,
            config.auth.refreshToken.options
